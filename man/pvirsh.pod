...
Today if you want to manage more than one VM you need to script you action and this will 
be done in a sequential way (most of the time). B<pvirsh> is a wrapper to launch async
management command on a group of VM.
...
in B</etc/pvirsh> directory. If you are using the setup.py it will be in B</usr/share/pvirsh/> directory.
Its possible to not use a group yaml by selecting vm from a list.
...
=item Launch parallel command on mulitiple VM selected by group (or in a list)

=item auto-completion of the virsh domain command

=item Reports of error/success per VM
...
=item B<show_xml_path>: choose current path to the XML device definition
...
=item B<select_vm>: select vm(s) to work on (multiple selection possible separate by ,)

=item B<cmd>: execute a domain command

=item B<hcmd>: help on virsh command (display options)

=item B<add_dev>: easy way to add a device to a group of VM

=item B<remove_dev>: easy way to remove a device to a group of VM

=item B<start>: start a group of VM

=item B<shutdown>: shutdown a group of VM
